...
function! friendlygrep#FriendlyGrep()
  ...
  let recursive = s:set_grep_recursive_option_with('grep recursively? [y/n] ')
  if s:return_flg == 1
    return
  endif
  ...
  try
    ...
    execute 'grep '.query.' . '.recursive.'--include='.target
  catch
    if g:friendlygrep_display_result_in == 'tab'
      tabclose
...
function! s:set_grep_recursive_option_with(prompt_msg)
